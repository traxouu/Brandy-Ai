...
export const metadata = { title: "Abonnement — Brandy AI" };
...
export default async function BillingPage({ searchParams }: Props) {
  ...
  const usagePercent =
    plan.projectLimit === -1
      ? 0
      : Math.min(100, Math.round((projectCount / plan.projectLimit) * 100));
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[1.3fr_1fr]">
        <div className="card p-8">
          ...
          <div className="mt-2 flex flex-wrap items-baseline gap-4">
            <h2 className="font-logo text-4xl">{plan.name}</h2>
            <span className="text-lg text-[var(--color-ink-muted)]">
              {plan.price === 0 ? "gratuit" : `${plan.price} € / mois`}
            </span>
          </div>
          <p className="mt-2 text-sm text-[var(--color-ink-soft)]">{plan.tagline}</p>
          ...
          <div className="mt-8">
            ...
            <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-[var(--color-cream-deep)]">
              <div
                className="h-full rounded-full bg-[var(--color-brand)] transition-all"
                style={{ width: `${plan.projectLimit === -1 ? 8 : usagePercent}%` }}
              />
            </div>
          </div>
        ...
        <div className="card p-8">
          ...
          <dl className="mt-4 space-y-4 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-[var(--color-ink-muted)]">Statut</dt>
              <dd className="font-bold">
                {user.planStatus === "demo" ? "démo" : user.planStatus}
              </dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-[var(--color-ink-muted)]">Prochaine échéance</dt>
              <dd className="font-bold">
            ...
            <div className="flex justify-between gap-4">
              <dt className="text-[var(--color-ink-muted)]">Compte</dt>
          ...
          <ul className="mt-3 space-y-2 text-sm text-[var(--color-ink-soft)]">
            {plan.features.map((feature) => (
              <li key={feature} className="flex gap-2.5">
                <span className="mt-[0.4em] block h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--color-brand)]" />
                {feature}
              </li>
            ))}
          </ul>
