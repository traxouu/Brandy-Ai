...
import { getPlan, formatLimit, planAllowsMoreProjects } from "@/lib/plans";
import { parseProject } from "@/lib/project";
import ProjectCard from "@/components/ProjectCard";
import { hasApiKey } from "@/lib/anthropic";

export const dynamic = "force-dynamic";
export const metadata = { title: "Mes projets — Brandy AI" };
...
export default async function DashboardPage() {
  ...
  const canCreate = planAllowsMoreProjects(user.plan, projects.length);
  const parsed = projects.map(parseProject);

  return (
    <>
      {!hasApiKey() && (
        <div className="mb-8 rounded-xl border border-[var(--color-brand)]/30 bg-[var(--color-brand-soft)] px-5 py-4 text-sm text-[var(--color-brand-dark)]">
      ...
      {plan.projectLimit !== -1 && (
        <div className="mt-6 h-1.5 w-full overflow-hidden rounded-full bg-[var(--color-cream-deep)]">
          <div
            className="h-full rounded-full bg-[var(--color-brand)] transition-all"
            style={{
              width: `${Math.min(100, (projects.length / plan.projectLimit) * 100)}%`,
            }}
          />
        </div>
      )}
      ...
      {parsed.length === 0 ? (
        <div className="card mt-12 px-8 py-20 text-center">
          <p className="font-logo text-5xl text-[var(--color-brand)]">01</p>
